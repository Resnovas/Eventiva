...
export class FeatureOfRoomRelationsResolver {
  ...
  async feature(@TypeGraphQL.Root() featureOfRoom: FeatureOfRoom, @TypeGraphQL.Ctx() ctx: any): Promise<Feature> {
  ...
  async room(@TypeGraphQL.Root() featureOfRoom: FeatureOfRoom, @TypeGraphQL.Ctx() ctx: any): Promise<Room> {
    return getPrismaFromContext(ctx).featureOfRoom.findUnique({
      where: {
        roomId_featureId: {
          roomId: featureOfRoom.roomId,
