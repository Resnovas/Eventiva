...
export class SIABadgeRelationsResolver {
  ...
  async siaChecks(@TypeGraphQL.Root() sIABadge: SIABadge, @TypeGraphQL.Ctx() ctx: any, @TypeGraphQL.Args() args: SIABadgeSiaChecksArgs): Promise<SIACheck[]> {
  ...
  async account(@TypeGraphQL.Root() sIABadge: SIABadge, @TypeGraphQL.Ctx() ctx: any): Promise<Account | null> {
    return getPrismaFromContext(ctx).sIABadge.findUnique({
      where: {
        id: sIABadge.id,
      },
