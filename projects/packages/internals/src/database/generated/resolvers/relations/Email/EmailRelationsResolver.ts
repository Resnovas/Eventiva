...
export class EmailRelationsResolver {
  ...
  async associatedContact(@TypeGraphQL.Root() email: Email, @TypeGraphQL.Ctx() ctx: any, @TypeGraphQL.Args() args: EmailAssociatedContactArgs): Promise<ContactAtAddress[]> {
  ...
  async domain(@TypeGraphQL.Root() email: Email, @TypeGraphQL.Ctx() ctx: any): Promise<Domain | null> {
    return getPrismaFromContext(ctx).email.findUnique({
      where: {
        id: email.id,
      },
  ...
  async account(@TypeGraphQL.Root() email: Email, @TypeGraphQL.Ctx() ctx: any): Promise<Account | null> {
    return getPrismaFromContext(ctx).email.findUnique({
      where: {
        id: email.id,
      },
