...
export class ArrangementOfRoomRelationsResolver {
  ...
  async arrangement(@TypeGraphQL.Root() arrangementOfRoom: ArrangementOfRoom, @TypeGraphQL.Ctx() ctx: any): Promise<Arrangement> {
  ...
  async room(@TypeGraphQL.Root() arrangementOfRoom: ArrangementOfRoom, @TypeGraphQL.Ctx() ctx: any): Promise<Room> {
    return getPrismaFromContext(ctx).arrangementOfRoom.findUnique({
      where: {
        roomId_arrangementId: {
          roomId: arrangementOfRoom.roomId,
