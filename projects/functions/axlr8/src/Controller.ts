...
import { SessionDetails } from './util/reports/EventSessionBooking';
/**
...
export class SIACheckController extends Controller {
  ...
  @Example<reportResult>({
    ...
    debug: {
      ...
      rptFilters: [
        {
          filter: SessionDetails['Number still required'],
          operator: '>',
          values: '0',
        },
        {
          filter: SessionDetails['Shift Date'],
          operator: '>',
          values: '09/03/2022',
        },
      ],
  ...
  public async getData(
    ...
    @Res() unauthorised: TsoaResponse<401, string>
  ): Promise<reportResult> {
    let api_pwd: string | undefined;
    if (api_user == process.env.API_USER) {
      if (token == process.env.TOKEN) api_pwd = process.env.API_PWD;
    }
    if (!api_pwd) return unauthorised(401, 'Unauthorised');
    ...
    if (result.success == false) return notFoundResponse(422, result);
    else return result;
  }
}
